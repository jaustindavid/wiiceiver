...
/*
     uint16_t x = 0;
     for (uint8_t i=0; i < (SSD1306_LCDWIDTH/8); i++) {
       for (uint8_t iprime = 0; iprime < 2; iprime ++) {
         for (uint8_t j=0; j < (SSD1306_LCDHEIGHT/8); j++) {
           if (x % 16 == 0) {
             Wire.beginTransmission(_i2caddr);
             WIRE_WRITE(0x40);
           }
           WIRE_WRITE(buffer[i + j * SSD1306_LCDHEIGHT/8]);
           if (x % 16 == 0) {
             Wire.endTransmission();
           }
           x++;
