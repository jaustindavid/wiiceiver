...
void updateLEDs(float throttlePos) {
  if (throttlePos == 0) {
    ...
    int bps = abs(int(throttlePos * 20));
...
float smoove(float target) {
  static Smoover upper(0.004, 0.005, 0.05, 0.003), 
                 downer(0.004, 0.005, 0.05, 0.003);
  float smoothed;
  
  Serial.print("Smooving: ");
  Serial.print(target);
  Serial.print("-> ");
  if (target > 0) {
    downer.smoove(0);
